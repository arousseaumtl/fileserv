...
class TestTokens(unittest.TestCase):
    ...
    def test_generate_and_validate_token_immediately(self):
        print("Testing if token is valid immediately after generation")
        self.assertTrue(
            Tokens.validate_token(self.token, self.unique_string, self.file_path),
    ...
    def test_generate_and_validate_token_within_validity(self):
        ...
        self.assertTrue(
            Tokens.validate_token(self.token, self.unique_string, self.file_path),
            "Token should still be valid within the validity period",
        )
    ...
    def test_generate_and_validate_token_after_expiry(self):
        ...
        self.assertFalse(
            Tokens.validate_token(self.token, self.unique_string, self.file_path),
            "Token should be invalid after the validity period",
        )
    ...
    def test_tampered_token(self):
        ...
        self.assertFalse(
            Tokens.validate_token(tampered_token, self.unique_string, self.file_path),
